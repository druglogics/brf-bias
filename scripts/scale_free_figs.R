...
sf_topo_stats %>%
  ...
  guides(fill = guide_legend(title = "Gamma")) +
  labs(y = "Number of equations with link operators", x = "Scale-free gamma parameter",
    title = "Scale-free topologies - link-operator equations") +
...
sf_topo_stats %>%
  ...
  scale_x_continuous(breaks = c(2,2.5)) +
  guides(fill = guide_legend(title = "Gamma")) +
  labs(y = "Maximum in-degree", x = "Scale-free gamma parameter",
    title = "Scale-free topologies - #regulators of biggest hub node") +
  theme_classic(base_size = 14)
...
sf_topo_stats %>%
  ...
  labs(x = "Scale-free gamma parameter", title = "Number of Stable states per abmlog-ensemble", y ="") + # for each abmlog-ensemble generated from a scale-free topology
  theme_classic(base_size = 14) +
...
sf_topo_stats %>%
  group_by(gamma) %>%
  summarise(zero_ss_per = sum(ss_total == 0)/n()) %>%
  ...
  scale_fill_brewer(palette = "Set1") +
  guides(fill = guide_legend(title = latex2exp::TeX("$\\gamma$"))) +
  labs(x = "Scale-free gamma parameter", title = "Zero stable states for all abmlog models",
...
sf_topo_stats %>%
  ...
  summarise(sum_ss = sum(ss_total)) %>%
    ...
    scale_fill_brewer(palette = "Set1") +
    guides(fill = guide_legend(title = latex2exp::TeX("$\\gamma$"))) +
    labs(x = "Scale-free gamma parameter", title = "Total number of stable states", y = "") +
    theme_classic(base_size = 14) +
    theme(plot.title = element_text(hjust = 0.5))
